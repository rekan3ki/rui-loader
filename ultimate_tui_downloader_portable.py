...
# IMPORTANT: Detect if running as .exe or .py
...
print(f"[INFO] Running from: {ROOT}")
print(f"[INFO] Data folder: {DATA_GAMES_PATH}")
...
def list_games() -> List[str]:
    if not DATA_GAMES_PATH.exists():
        print(f"\033[91m[!] Games folder not found: {DATA_GAMES_PATH}\033[0m")
        return []
...
def main():
    ...
    if not games:
        ...
        print(f"\033[91m[!] Expected path: {DATA_GAMES_PATH}\033[0m")
        print(f"\033[93m[!] Make sure folder structure is:\033[0m")
        print(f"    {ROOT}/")
        print(f"    ├── data/")
        print(f"    │   └── games/")
        print(f"    ├── config/")
